...
const AdminInvoicesTab = ({
  ...
  const resetBuilder = () => {
    ...
    setBuilderNumber(`INV-2026-${String(invoices.length + 1).padStart(4, "0")}`);
  ...
  const appendLineItem = () => {
    if (!itemDesc.trim() || itemPrice <= 0) {
      showToast("Please provide item description and price.", "warning");
      return;
    }
  ...
  const handleSaveInvoice = (e) => {
    ...
    if (editingInvoice) {
      ...
      showToast(`Invoice ${editingInvoice.id} updated.`, "success");
    } else {
      const newInv = {
        id: builderNumber,
        client: builderClient,
      ...
      showToast(`Invoice ${builderNumber} created as Draft.`, "success");
    }
    resetBuilder();
  };
  ...
  const handleEditClick = (inv) => {
    ...
    setBuilderItems(inv.itemsList || [{ desc: "Base Services", qty: 1, price: inv.amount }]);
  ...
  const handleDuplicate = (inv) => {
    const duplicatedNum = `INV-COPY-${Date.now().toString().slice(-4)}`;
    const duplicatedInv = {
      ...inv,
      id: duplicatedNum,
      date: new Date().toISOString().split("T")[0],
    ...
    showToast(`Duplicated ${inv.id} to new Draft ${duplicatedNum}.`, "info");
  };

  const handleDelete = (id, status) => {
    if (status === "paid") {
      showToast("❌ Compliance Lock: Paid invoices cannot be deleted from ledger.", "error");
      return;
    }
  ...
  const downloadPrintHtml = (inv) => {
    ...
    if (!printWindow) {
      showToast("Pop-up blocker active. Allow popups to preview.", "error");
      return;
    }

    const items = inv.itemsList || [{ desc: "Billing consulting", qty: 1, price: inv.amount }];
      ...
      <html>
        ...
        <body>
          <div class="header-panel">
            <div>
              <div class="brand">BillNest Invoice</div>
          ...
          <div class="footer">
            BillNest Multi-Tenant SaaS Cloud Invoicing Ecosystem • Immutable Ledger
          </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-7 space-y-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-4">
          <div className="flex justify-between items-center border-b border-slate-50 pb-3">
            <h4 className="font-heading text-xs font-bold text-slate-800 uppercase tracking-wider">Invoice History</h4>
          ...
          <div className="space-y-3.5">
            {filteredInvoices.length === 0 ? (
              <div className="text-center py-12 text-slate-400">
                ...
                <h5 className="font-heading font-bold text-sm text-slate-800 mt-2">No Invoices Available</h5>
                <p className="text-[10px] text-slate-400 max-w-xs mx-auto mt-1 leading-normal">
                  No records matched this active segment. Add line items to draft billing logs.
                </p>
              </div>
            ) : (
              filteredInvoices.map(inv => {
                ...
                return (
                  <div key={inv.id} className="p-4 bg-slate-50/50 rounded-2xl border border-slate-100 flex flex-col sm:flex-row justify-between gap-4 items-start sm:items-center hover:border-slate-200 transition-all">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {inv.status === "sent" && (
                          <button
                            ...
                            className="bg-emerald-600 hover:bg-emerald-700 text-white text-[9px] px-2 py-1 rounded-lg font-black uppercase transition-all cursor-pointer shadow-sm shadow-emerald-100"
                          >
                            Mark Paid
                          </button>
                        )}
                        ...
                        <button
                          ...
                          title={isPaid ? "Paid invoices cannot be deleted" : "Delete"}
                        >
                          Delete
                        </button>
                      </div>
      ...
      {/* Right side: Creator / Editor Panel */}
      <div className="lg:col-span-5 bg-white p-5 rounded-3xl border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
        <h4 className="font-heading text-xs font-bold text-slate-800 uppercase tracking-wider border-b border-slate-50 pb-2 mb-4">
          {editingInvoice ? `Editing ${editingInvoice.id}` : "Draft Corporate Bill"}
        </h4>
        ...
        <form onSubmit={handleSaveInvoice} className="space-y-4">
          ...
          <div className="p-3.5 bg-slate-50/50 border border-slate-100 rounded-2xl space-y-3">
            <span className="block font-heading text-[10px] font-black text-slate-400 uppercase tracking-widest">Append Line Items</span>
          ...
          <div className="flex gap-2">
            ...
            >
              {editingInvoice ? "Save Invoice" : "Draft Invoiced Bill"}
            </button>
            <button
              type="button"
