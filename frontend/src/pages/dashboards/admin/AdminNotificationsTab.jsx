...
const AdminNotificationsTab = ({
  ...
  const markAllAsRead = () => {
    ...
    showToast("All workspace alerts marked as read.", "success");
  };

  const clearAllNotifications = () => {
    setNotifications([]);
    showToast("Notifications center cleared.", "info");
  };
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case "team_invitation":
        return { symbol: "group", color: "text-amber-600 bg-amber-50 border-amber-100" };
      default:
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)] flex flex-wrap gap-2 text-xs font-semibold select-none">
          {[
            ...
            { id: "payment_failed", label: "Payment Failure" },
            { id: "subscription_renewal", label: "Subscription Renewals" },
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-4">
          <div className="flex justify-between items-center border-b border-slate-50 pb-3">
            <div>
              <h4 className="font-heading text-xs font-bold text-slate-800 uppercase tracking-wider">Inbox Notifications</h4>
              <span className="text-[10px] text-slate-400 font-semibold mt-0.5 block">Workspace telemetry updates</span>
            </div>
          ...
          <div className="space-y-3.5">
            {filteredNotifications.length === 0 ? (
              <div className="text-center py-12 text-slate-400">
                ...
                <h5 className="font-heading font-bold text-sm text-slate-800 mt-2">No Alerts Dispatched</h5>
                <p className="text-[10px] text-slate-400 max-w-xs mx-auto mt-1 leading-normal">
                  All telemetry modules report a healthy baseline. Select alternative categories.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 font-semibold leading-relaxed mt-1">{item.message}</p>
      ...
      {/* Right side: Telemetry Statistics */}
