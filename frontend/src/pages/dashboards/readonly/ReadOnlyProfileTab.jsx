...
const ReadOnlyProfileTab = ({
  ...
  const handleSaveProfile = (e) => {
    e.preventDefault();
    showToast("Profile credentials and alert configurations stored.", "success");
  };
  ...
  const handleUpdatePassword = (e) => {
    ...
    showToast("Security password updated securely.", "success");
    setOldPassword("");
    setNewPassword("");
    setConfirmPassword("");
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      {/* Left side: Credentials form & Notification preferences */}
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-6">
          <div className="flex justify-between items-center border-b border-slate-50 pb-2 select-none">
            ...
            <span className="px-2.5 py-0.5 bg-indigo-50 border border-indigo-100 rounded text-[9px] font-black text-indigo-700 uppercase tracking-wider">
              {user?.role?.toUpperCase() || "READ ONLY"} PRIVILEGES
            </span>
          </div>
          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center font-extrabold text-white text-2xl shadow-md border border-indigo-100 select-none">
            ...
            <div className="pt-4 border-t border-slate-50 space-y-3">
              <span className="block text-slate-500 text-[10px] font-bold uppercase tracking-wider">Alert Subscriptions</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-semibold text-slate-600">
                {[
                  ...
                  { key: "invoices", label: "Invoice Alerts" },
                  { key: "team", label: "Team Activity Alerts" },
                  { key: "system", label: "System Notifications" }
                ].map(pref => (
                  <label key={pref.key} className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      type="checkbox"
      ...
      <div className="lg:col-span-5 bg-white p-6 rounded-3xl border border-slate-100 shadow-[0_1px_3px_rgba(0,0,0,0.02)] space-y-4">
        ...
        <form onSubmit={handleUpdatePassword} className="space-y-3.5">
          ...
          <button
            type="submit"
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-xl text-xs font-bold shadow-sm transition-colors cursor-pointer uppercase tracking-wider animate-pulse"
          >
            Modify Password
          </button>
        </form>
...
export default ReadOnlyProfileTab;
